...
bool resuelveCaso() {
	...
	while (order != "FIN") {
		...
		try {
			...
			else if (order == "recuperar") {

				std::cin >> DNI >> points;

				DGT.recover_points(DNI, points);
			}
			...
			else if (order == "lista_por_puntos") {

				std::cin >> points;

				std::list<std::string> dni_list = DGT.list_to_points(points);

				std::cout << "Tienen " << points << " puntos:";

				for (auto const & d_l : dni_list) std::cout << ' ' << d_l;
